<?php
$title = $attributes['title'] ?? 'Why Join Datum';
...
<div class="bg-gradient-to-r from-secondary to-primary text-white">
  <div class="container flex flex-col gap-8 py-[40px] lg:py-[100px]">
    <div>
      <h2 class="text-[24px] font-bold lg:text-[36px] lg:pb-0 pb-[10px]">
        <?php echo esc_html($title); ?>
      </h2>
      <?php if ($heading): ?>
        <p class="text-lg lg:text-xl font-light"><?php echo esc_html($heading); ?></p>
      <?php endif; ?>
      <?php if ($content): ?>
        <div class="mt-4 text-base leading-relaxed"><?php echo wp_kses_post($content); ?></div>
      <?php endif; ?>
    </div>
