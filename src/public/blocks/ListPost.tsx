...
const ListPost: React.FC<ListPostProps> = (props) => {
    const category = props.data?.attributes?.category ?? '';
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            try {
                const url = `/index.php?rest_route=/datum/v1/get-posts` + (category ? `&category=${encodeURIComponent(category)}` : '');
        ...
        <div className="container pt-[60px] lg:pt-[80px]">
            ...
            {totalPages > 1 && (
                <div className="flex justify-center mt-8 space-x-2">
                    ...
                    >
                        <ArrowRightOutlined rotate={180} style={{ fontSize: 16, color: "#2563eb", fontWeight: 'bold' }} />
                    </a>
                    {[...Array(totalPages)].map((_, idx) => (
                        <a
                            key={idx}
                            href="#"
                            className={`px-1 inline-block font-bold ${currentPage === idx + 1 ? 'border-b-2 border-blue-700 text-blue-700' : 'text-gray-700'}`}
                            onClick={e => {
                                e.preventDefault();
                    ...
                    >
                        <ArrowRightOutlined style={{ fontSize: 16, color: "#2563eb", fontWeight: 'bold' }} />
                    </a>
                </div>
            )}
        </div>
